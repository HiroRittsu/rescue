...
public class SampleRoadDetector extends RoadDetector {
	...
	public RoadDetector calc() {
		 ...
		 * this.agentInfo.getPosition(); System.out.println(this.targetAreas); if
		 * (this.targetAreas.contains(positionID)) { this.result = positionID; return
		 * this; } List<EntityID> removeList = new
		 * ArrayList<>(this.priorityRoads.size()); for (EntityID id :
		 * this.priorityRoads) { if (!this.targetAreas.contains(id)) {
		 * removeList.add(id); } } this.priorityRoads.removeAll(removeList); if
		 * (this.priorityRoads.size() > 0) { this.pathPlanning.setFrom(positionID);
		 * this.pathPlanning.setDestination(this.targetAreas); List<EntityID> path =
		 * this.pathPlanning.calc().getResult(); if (path != null && path.size() > 0) {
		 * this.result = path.get(path.size() - 1); } return this; }
		 * 
